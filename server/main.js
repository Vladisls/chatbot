    ...
    request.on('response', Meteor.bindEnvironment(function(response) {
      ...
      switch(command[0]) {
        ...
        case "TRANSLATE":
            ...
            if (response.result.parameters.language == ""){
                targetLanguage = 'en';
            }else{
                targetLanguage = getIso(response.result.parameters.language);
            }
            translate({
                text: response.result.parameters.any,
                source: 'auto',
...
function sendMessageAsBot(message){
    Messages.insert({
        message: message,
        timestamp: new Date(),
        user:'dffawde21edfasd',
        username: 'gotoAndBot'
    });      
  }

// Map of language ISO's
var isoLangs = {
		"ab":{
			"name":"Abkhaz",
			"nativeName":"аҧсуа"
...
// Function that returns language iso from language name
function getIso(string){
    lang = '';
    Object.keys(isoLangs).forEach(function(key) {
        if (string.toLowerCase() == isoLangs[key]["name"].toLowerCase()){
            lang = key;
        }
    });
    if (lang == ''){
